...
module.exports = async (req, res) => {
  ...
  if (!API_KEY || API_KEY === 'sk_solrouter_YOURKEYHERE')
    return res.status(500).json({ error: 'SolRouter API key not configured' });
  ...
  try {
    ...
    const totalUsd = (solPrice * portfolio.solBalance) + enrichedTokens.reduce((s, t) => s + (t.usdValue || 0), 0);
    res.status(200).json({
      portfolio: { ...portfolio, tokens: enrichedTokens, solUsdValue: solPrice * portfolio.solBalance, solPrice, totalUsd },
      report,
      approvals,
    });
  } catch (err) {
